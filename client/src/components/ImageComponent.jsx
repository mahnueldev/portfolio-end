...
// Component
const ImageComponent = ({ src, alt, name, preview, remove }) => {
  const [hovered, setHovered] = useState(false);

  return (
    <section className='relative inline-block rounded xl:w-5/6 xl:h-max md:w-3/6 h-max' onMouseEnter={() => setHovered(true)}
    onMouseLeave={() => setHovered(false)}>
      ...
      {( 
        <section className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-100'>
          <button className='bg-transparent border-none cursor-pointer  transition-transform duration-200 ease-in-out ' onClick={preview}>
          ...
          <button
            ...
            style={{ marginLeft: '10px' }} onClick={remove}
          >
            <DeleteOutlined style={{ fontSize: '26px', color: '#314ffc' }} />
          </button>
        </section>
      )}
      <div className='text-center mt-2 text-sm font-bold text-dark-300'>{name}</div>
    </section>
  );
};
